...
public class CopyOfAdaptiveInterestRateAverageThreshold extends AbstractStrategy implements
	...
	public double computeInterestRate(MacroAgent creditDemander, double amount,
		...
		else if (mktId==StaticValues.MKT_COCOBONDS){
			double tot=0;
			double inter=0;
			for (Agent b:banks.getAgents()){
				Bank bank = (Bank) b;
				tot+=bank.getLiquidityRatio(); /*VARIABLE DE REF A MODIFIER SANS DOUTE -> EN FAIT NON, on va dire que le taux doit pas dépasser une certaine valeur qui viendrait pénaliser les profits et doncs le ratio de liquidité*/
				inter+=bank.getPassedValue(StaticValues.LAG_COCOBONDINTEREST, 1);
				}
			threshold=tot/banks.getSize();
			avInterest=inter/banks.getSize();
		}
		...
		if (mktId==StaticValues.MKT_DEPOSIT){
			...
			referenceVariable=lender.getLiquidityRatio(); /*VARIABLE DE REF A MODIFIER SANS DOUTE*/
		}
