...
import jmab.strategies.MostPayingCoCoBondsWithSwitching;
...
public class SFCSSMacroAgentInitialiser extends AbstractMacroAgentInitialiser implements MacroAgentInitialiser{
	...
	public void initialise(MacroPopulation population, MacroSimulation sim) {	
		...
		double hhCoCobonds = this.hhsCoCoBonds/hhSize;
		...
		for(int i = 0; i<hhSize; i++){
			...
			Bank bank = (Bank) banks.getAgentList().get(bankId);
			int cocobondMat = bank.getCoCoBondMaturity();
			int nbCoCoBondsPerPeriod = (int) (hhCoCobonds/(cocobondMat*bsPrice));
			for(int n = 1 ; n<=cocobondMat; n++) {
				CoCoBond cocobond = new CoCoBond(nbCoCoBondsPerPeriod*bsPrice, nbCoCoBondsPerPeriod, hh, bank, bank.getCoCoBondMaturity(), this.iCoCoBonds, bsPrice);
				cocobond.setAge(n);
				hh.addItemStockMatrix(cocobond, true, StaticValues.SM_COCOBONDS);
				bank.addItemStockMatrix(cocobond, false, StaticValues.SM_COCOBONDS);
			}
			...
			//Set Previous CoCoBond Supplier
			MostPayingCoCoBondsWithSwitching cocobondStrategy= (MostPayingCoCoBondsWithSwitching) hh.getStrategy(StaticValues.STRATEGY_COCOBOND);
			CoCoBondSupplier previousCoCoBondSupplier= (CoCoBondSupplier) banks.getAgentList().get(bankId);
			cocobondStrategy.setPreviousCoCoBondSupplier(previousCoCoBondSupplier);
		...
		//Banks
		double lagHHCoCoBond=hhCoCobonds*(1+distr.nextDouble());
		double lagHHCoCoBondInterestReceived=iCoCoBonds*lagHHCoCoBond*(1+distr.nextDouble());
		...
		for(int i = 0; i<bSize; i++){
			...
			b.addValue(StaticValues.LAG_COCOBONDSPRICE, bsPrice);
			b.addValue(StaticValues.LAG_COCOINTERESTSPAID,lagHHCoCoBondInterestReceived);
			b.addValue(StaticValues.LAG_COCOISSUED, lagHHCoCoBond);	
			b.addValue(StaticValues.LAG_COCOBONDINTEREST,iCoCoBonds);
			b.addValue(StaticValues.LAG_COCOTRIGGERED, 0*(1+distr.nextDouble()));
			double div = b.getDividends();
			b.addValue(StaticValues.LAG_DIVIDENDS, div*(1+distr.nextDouble()));
	...
	/**
	 * @return the hhsCoCoBond
	 */
	public double getHhsCoCoBonds() {
		return hhsCoCoBonds;
	}

	/**
	 * @param hhsCoCoBonds the hhsCoCoBonds to set
	 */
	public void setHhsCoCoBonds(double hhsCoCoBonds) {
		this.hhsCoCoBonds = hhsCoCoBonds;
	}
	...
	/**
	 * @param cocobondPrice the cocobondPrice to set
	 */
	...
	/**
	 * @return the iCoCoBonds
	 */
	public double getiCoCoBonds() {
		return iCoCoBonds;
	}

	/**
	 * @param iCoCoBonds the iCoCoBonds to set
	 */
	public void setiCoCoBonds(double iCoCoBonds) {
		this.iCoCoBonds = iCoCoBonds;
	}
