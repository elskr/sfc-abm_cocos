...
spline_finalDataSet[,c(3:4)] <- sapply(spline_finalDataSet[,c(3:4)], as.numeric)
...
lm_mod1 <- lm(Consumption ~ factor(threshold) + CoCos, data = spline_finalDataSet )
lm_mod2 <- lm(GDP ~ factor(threshold) + CoCos, data = spline_finalDataSet )
lm_mod3 <- lm(Investment ~ factor(threshold) + CoCos, data = spline_finalDataSet )
...
quad_mod1 <- lm(Consumption ~ factor(test) + CoCos + I(CoCos^2), data = spline_finalDataSet )
quad_mod2 <- lm(GDP ~ factor(test) + CoCos + I(CoCos^2), data = spline_finalDataSet )
quad_mod3 <- lm(Investment ~ factor(test) + CoCos + I(CoCos^2), data = spline_finalDataSet )
...
gamm_modA <- gamm (Consumption ~  s(CoCos),data = spline_finalDataSet, random=list(threshold=~1), correlation = corARMA( p = 1, q = 0 ) )
summary(gamm_modA$gam)
#gamtabs(gamm_modA$gam)
itsadug::plot_smooth(gamm_modA$gam, view="CoCos", rug=F)

gamm_modB <- gamm (GDP ~  s(CoCos),data = spline_finalDataSet, random=list(threshold=~1), correlation = corARMA( p = 1, q = 0 ))
summary(gamm_modB$gam)
#gamtabs(gamm_modB$gam)
itsadug::plot_smooth(gamm_modB$gam, view="CoCos", rug=F)
