 ...
 *
 */
public class HouseholdsCoCoBondsComputer extends AbstractMicroComputer implements MicroMultipleVariablesComputer {
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:pop.getAgents()){
			...
			if (!agent.isDead()){
			List<Item>loans=agent.getItemsStockMatrix(true, cocobondsId);
			double cocos=0;
			for (Item j:loans){
				CoCoBond coco= (CoCoBond)j;
				cocos+=coco.getValue();
				}
			result.put(agent.getAgentId(), cocos);
			}
			else{
